 ...
 *
 * Usage:
 *   adb shell am broadcast -a io.agents.arya.TASK --es task "send hi to Mom on WhatsApp" -p io.agents.arya
 *   adb shell am broadcast -a io.agents.arya.TASK --es chat "read my clipboard and explain what it says" -p io.agents.arya
 *   adb shell am broadcast -a io.agents.arya.TASK --es chat_b64 "$(printf 'remember token abc123 and reply with only OK' | base64 -w0)" -p io.agents.arya
 *
...
public class TaskTriggerReceiver extends BroadcastReceiver {
    ...
    public static final String ACTION = "io.agents.arya.TASK";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (hasTask && task.startsWith("autoreply ")) {
            ...
            if (cmd.equals("off")) {
                io.agents.arya.service.AutoReplyManager.getInstance().stopAll();
