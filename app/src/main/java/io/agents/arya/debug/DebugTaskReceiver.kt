 ...
 *
 * Usage:
 *   adb shell am broadcast -a io.agents.arya.DEBUG_TASK --es task "open my camera"
 *
 * Set Cloud LLM config (any provider):
 *   adb shell am broadcast -a io.agents.arya.DEBUG_TASK --es task "config:" \
...
class DebugTaskReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (!io.agents.arya.BuildConfig.DEBUG) return
            ...
            try {
                ...
                if (provider == "LOCAL") {
                    ...
                    val resolvedBaseUrl = baseUrl
                        ?: io.agents.arya.agent.CloudProvider.findProviderForModel(modelName ?: "")?.defaultBaseUrl
                ...
                val vm = io.agents.arya.ClawApplication.appViewModelInstance
    ...
    private fun executeTool(intent: Intent, toolName: String, paramsJson: String) {
        try {
            ...
            breadcrumb(io.agents.arya.ClawApplication.instance, "executeTool start tool=$toolName params=$params")
            ...
            if (result.isSuccess) {
                ...
                breadcrumb(io.agents.arya.ClawApplication.instance, "executeTool success tool=$toolName data=${result.data}")
            } else {
                XLog.w("DebugTaskReceiver", "Debug tool failed: $toolName -> ${result.error}")
                breadcrumb(io.agents.arya.ClawApplication.instance, "executeTool fail tool=$toolName error=${result.error}")
            }
        } catch (e: Exception) {
            XLog.e("DebugTaskReceiver", "Failed to execute debug tool", e)
            breadcrumb(io.agents.arya.ClawApplication.instance, "executeTool exception tool=$toolName error=${e.message}")
        }
    }
    ...
    private fun simulateIncomingMessage(intent: Intent, simulateMessage: String) {
        try {
            ...
            breadcrumb(
                io.agents.arya.ClawApplication.instance,
        ...
        try {
            when (action.lowercase()) {
                "status" -> {
                    ...
                    breadcrumb(io.agents.arya.ClawApplication.instance, "backend status ${LocalBackendHealth.debugStateSummary()}")
                }
                "force_cpu_safe" -> {
                    ...
                    breadcrumb(io.agents.arya.ClawApplication.instance, "backend force_cpu_safe reason=$reason ${LocalBackendHealth.debugStateSummary()}")
                }
                "clear_cpu_safe" -> {
                    ...
                    breadcrumb(io.agents.arya.ClawApplication.instance, "backend clear_cpu_safe ${LocalBackendHealth.debugStateSummary()}")
                }
                "mark_pending_gpu_init" -> {
                    ...
                    breadcrumb(io.agents.arya.ClawApplication.instance, "backend mark_pending_gpu_init model=$modelPath ${LocalBackendHealth.debugStateSummary()}")
                }
                "clear_pending_gpu_init" -> {
                    ...
                    breadcrumb(io.agents.arya.ClawApplication.instance, "backend clear_pending_gpu_init ${LocalBackendHealth.debugStateSummary()}")
                }
                "recover_pending_gpu_crash" -> {
                    ...
                    breadcrumb(io.agents.arya.ClawApplication.instance, "backend recover_pending_gpu_crash recovered=$recovered ${LocalBackendHealth.debugStateSummary()}")
                }
                else -> {
                    XLog.w("DebugTaskReceiver", "Unknown backend_action=$action")
                    breadcrumb(io.agents.arya.ClawApplication.instance, "backend unknown action=$action")
                }
            }
        } catch (e: Exception) {
