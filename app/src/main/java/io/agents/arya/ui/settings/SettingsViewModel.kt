...
import io.agents.arya.widget.QRCodeDialog
...
class SettingsViewModel : ViewModel() {
    ...
    fun startWeChatQrLogin(context: Context) {
        viewModelScope.launch {
            val loadingDialog = io.agents.arya.widget.LoadingDialog.show(
                context = context,
                message = context.getString(R.string.channel_config_wechat_scanning)
            )
            try {
                val apiClient = io.agents.arya.channel.wechat.WeChatApiClient()
    ...
    private fun startWeChatQrPolling(
        context: Context,
        dialog: QRCodeDialog,
        apiClient: io.agents.arya.channel.wechat.WeChatApiClient,
        qrcode: String
    ): Job {
    ...
    fun unbindWeChat() {
        ...
        io.agents.arya.channel.wechat.WeChatInbound.clearContextTokensForAccount(accountId)
