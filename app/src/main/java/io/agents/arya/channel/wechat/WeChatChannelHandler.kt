...
import io.agents.arya.channel.ChannelHandler
...
class WeChatChannelHandler(
    ...
    private fun processInboundMessage(msg: WeChatMessage) {
        ...
        if (body.isEmpty()) {
            ...
            val app = io.agents.arya.ClawApplication.instance
            val mediaTypes = msg.itemList?.filter { WeChatInbound.isMediaItem(it) }?.map {
                when (it.type) {
                    MessageItemType.IMAGE -> app.getString(io.agents.arya.R.string.wechat_media_image)
                    MessageItemType.VIDEO -> app.getString(io.agents.arya.R.string.wechat_media_video)
                    MessageItemType.FILE -> app.getString(io.agents.arya.R.string.wechat_media_file)
                    MessageItemType.VOICE -> app.getString(io.agents.arya.R.string.wechat_media_voice)
                    else -> app.getString(io.agents.arya.R.string.wechat_media_unknown)
                }
            } ?: emptyList()
            if (mediaTypes.isNotEmpty()) {
                XLog.i(TAG, "Pure media message received: types=$mediaTypes, from=${fromUserId.takeLast(16)}")
                val tip = app.getString(io.agents.arya.R.string.wechat_unsupported_media, mediaTypes.joinToString("+"))
