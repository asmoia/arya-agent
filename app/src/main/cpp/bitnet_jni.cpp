 ...
 *
 * CRITICAL FIX (v0.5.4): do_completion was re-processing ALL tokens on every
 * generation step (O(n2)), making it ~200x slower than correct incremental
 * decoding (O(n)). Now fixed: prompt processed once, then each new token
 * processed individually using the KV cache.
 */
 ...
 *
 * STEP 1: Process full prompt ONCE (fills KV cache)
 * STEP 2: For each new token, process only THAT token (uses KV cache)
 *
 * Old bug was passing ALL tokens to llama_decode on every iteration,
 * causing O(n2) instead of O(n). A 500-token prompt generating 256 tokens
 * took ~160,000 processing steps instead of ~756. That is ~200x slower!
 */
    ...
    const std::vector<std::string> & stop_strs)
{
    ...
    if (n_prompt >= n_ctx) {
        LOGE("Prompt too long: %d tokens, context size: %d", n_prompt, n_ctx);
        return "";
    }

    int max_gen = max_tokens;
    if (n_prompt + max_gen >= n_ctx) {
        max_gen = n_ctx - n_prompt - 1;
    }
    if (max_gen <= 0) {
        LOGE("No room for generation after prompt (%d tokens in %d context)", n_prompt, n_ctx);
        return "";
    }
    ...
    LOGI("Prompt processed: %d tokens, generating up to %d tokens", n_prompt, max_gen);

    std::string result;
    int n_decoded = 0;

    // STEP 2: Generate tokens incrementally (one at a time, using KV cache)
    for (int i = 0; i < max_gen; i++) {
        ...
        // Process ONLY the new token (uses KV cache for all previous tokens)
        // THIS IS THE KEY FIX - before, ALL tokens were passed every time!
    ...
    llama_sampler_free(smpl);
    LOGI("Generated %d tokens from %d-token prompt (total context: %d)",
         n_decoded, n_prompt, n_prompt + n_decoded);
    return result;
}
